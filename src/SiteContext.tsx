...
export interface StrategicAxisSettings {
  number: string;
  title: string;
  subtitle: string;
  icon: string;
...
export interface SupportTierSettings {
  amount: number;
  currency: string;
  title: string;
  description: string;
...
export interface SiteSettingsData {
  ...
  about: {
    ...
    quoteText: string;
    quoteAuthor: string;
    strategicAxes: StrategicAxisSettings[];
  };
  actionsPage: {
    bannerTitle: string;
    bannerSubtitle: string;
    steps: ActionStepSettings[];
  };
  supportPage: {
    title: string;
    subtitle: string;
    tiers: SupportTierSettings[];
  };
  contact: {
    phone: string;
    email: string;
...
export const defaultSiteSettings: SiteSettingsData = {
  ...
  about: {
    ...
    strategicAxes: [
      {
        number: '01',
        title: 'Santé Reproductive & SDSR',
        subtitle: 'Promotion de la Santé et des Droits Sexuels et Reproductifs',
        icon: 'Heart',
        description: 'Éducation complète à la sexualité, sensibilisation à la santé menstruelle et accès aux services de santé de qualité pour tous.',
        color: '#2764AE'
      },
      {
        number: '02',
        title: 'Prévention des VBG',
        subtitle: 'Lutte contre les Violences Basées sur le Genre',
        icon: 'ShieldCheck',
        description: 'Prise en charge psychosociale, accompagnement juridique et campagnes de plaidoyer communautaire pour briser les tabous.',
        color: '#C49A45'
      },
      {
        number: '03',
        title: 'Autonomisation & Leadership',
        subtitle: 'Autonomisation Économique et Engagement des Jeunes',
        icon: 'Zap',
        description: 'Formations professionnelles, soutien aux micro-entreprises locales, pépinières écologiques et développement du leadership féminin.',
        color: '#27B074'
      },
      {
        number: '04',
        title: 'Paix & Justice Climatique',
        subtitle: 'Résilience Écologique et Cohésion Sociale',
        icon: 'Globe',
        description: 'Actions de sensibilisation environnementale, projets d’adaptation climatique communautaire et prévention des conflits.',
        color: '#0d1b2a'
      }
    ]
  },
    ...
    steps: [
      {
        ...
        title: 'ÉDUQUER',
        subtitle: "pour libérer l'esprit",
        color: '#2764AE',
      ...
      },
      {
        ...
        title: 'PROTÉGER',
        subtitle: 'pour préserver la dignité',
        color: '#C49A45',
      ...
      {
        stepNumber: '03',
        stepLabel: 'Troisième Étape',
        title: 'AUTONOMISER',
        subtitle: "pour bâtir l'avenir",
        color: '#27B074',
    ...
    tiers: [
      {
        ...
        description: 'Kit d’hygiène menstruelle pour 2 jeunes filles',
        benefits: ['Achat de serviettes réutilisables', 'Sensibilisation aux soins d’hygiène'],
        highlight: false
      },
      {
        amount: 25000,
        currency: 'FCFA',
        title: 'Support d’Impact',
        description: 'Prise en charge médicale d’une survivante de VBG',
        benefits: ['Consultation et soins médicaux', 'Accompagnement psychologique'],
        highlight: true
      },
      {
        amount: 100000,
        currency: 'FCFA',
        title: 'Grand Partenaire',
        description: 'Organisation d’une causerie sur les VBG',
        benefits: ['Animation communautaire complète', 'Distribution de supports de sensibilisation'],
        highlight: false
      }
    ]
  },
...
export const SiteProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSettings({
          ...defaultSiteSettings,
          ...data,
          colors: { ...defaultSiteSettings.colors, ...data.colors },
          typography: { ...defaultSiteSettings.typography, ...data.typography },
          hero: { ...defaultSiteSettings.hero, ...data.hero },
          sloganBanner: { ...defaultSiteSettings.sloganBanner, ...data.sloganBanner },
          about: { ...defaultSiteSettings.about, ...data.about },
          actionsPage: { ...defaultSiteSettings.actionsPage, ...data.actionsPage },
          supportPage: { ...defaultSiteSettings.supportPage, ...data.supportPage },
          contact: { ...defaultSiteSettings.contact, ...data.contact },
          footer: { ...defaultSiteSettings.footer, ...data.footer }
        });
      }
    } catch (err) {
      console.warn('Utilisation des paramètres par défaut pour Busola:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Injection dynamique des variables CSS sur document.documentElement
  useEffect(() => {
    if (settings && settings.colors) {
      const root = document.documentElement;
      root.style.setProperty('--brand-primary', settings.colors.primary);
      root.style.setProperty('--brand-secondary', settings.colors.secondary);
      root.style.setProperty('--brand-sable', settings.colors.sable);
      root.style.setProperty('--brand-cobalt', settings.colors.cobalt);
      root.style.setProperty('--brand-gold', settings.colors.gold);
      root.style.setProperty('--brand-vert', settings.colors.vert);
      root.style.setProperty('--brand-dark', settings.colors.dark);
      root.style.setProperty('--brand-light', settings.colors.light);
